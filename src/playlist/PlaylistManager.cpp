...
std::vector<TrackRename> PlaylistManager::ApplyScan(const library::LibraryScanResult& scan) {
    const auto renames = DetectRenames(scan.tracks);
    for (const auto& rename : renames) {
        // Remap every playlist reference and the old catalog/external record to the
        // renamed track. ReplaceLibrary below then installs the new scan wholesale, so
        // the bridge's lasting effect is the rewritten references (ids changed).
        (void)ReplaceTrack(rename.oldId, rename.replacement);
    }
