...
bool CopyPackage(const std::filesystem::path& source,
                 const std::filesystem::path& destination,
                 std::string* error) {
    std::error_code ec;
    std::filesystem::create_directory(destination, ec);
    if (ec) {
        SetError(error, "Unable to create skin staging directory: " + ec.message());
        return false;
    }
    ...
    if (ec) {
        SetError(error, "Unable to enumerate skin package: " + ec.message());
        return false;
    }
    ...
    for (; iterator != end; iterator.increment(ec)) {
        ...
        if (ContainsReparsePoint(source, entry, error)) {
            return false;
        }

        const auto relative = std::filesystem::relative(entry.path(), source, ec);
        if (ec || relative.empty() || relative.is_absolute()) {
            SetError(error, "Skin package contains an invalid path");
            return false;
        }
        ...
        if (entry.is_directory(ec)) {
            ...
            if (ec) {
                SetError(error, "Unable to create staged skin directory: " + ec.message());
                return false;
            }
            continue;
        }
        if (ec || !entry.is_regular_file(ec) || ec) {
            SetError(error, "Skin package may contain only directories and regular files");
            return false;
        }

        const auto size = entry.file_size(ec);
        if (ec || size > kMaximumPackageBytes - totalBytes || ++fileCount > kMaximumPackageFiles) {
        ...
        if (ec) {
            SetError(error, "Unable to stage skin file: " + ec.message());
            return false;
        }
    }
    return true;
